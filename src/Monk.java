...
public class Monk {
    Monk(Stats stat, String ancestry) {
...
//----------------------------------------------------------------------------------------------------------------------
    ...
    public void fileOutput() throws IOException {//this whole thing prints the class to a .txt file
        ...
        print.write("Name: " + Character.seedString.stripTrailing() + "        " + ancestry + " Monk " + level + "        Background: " + background + "\n\n");//adds name, ancestry, class, and level
        ...
        int unarmedStrikeBonus = stat.getMods()[1] + profBonus;
        print.write("Unarmed Strike  ");
        if (unarmedStrikeBonus >= 0) {
            print.write("+");
        }
        print.write(unarmedStrikeBonus + "   1d" + martialArtsDie);
        if (stat.getMods()[1] >= 0) {
            print.write("+");
        }
        print.write(stat.getMods()[1] + " bludgeoning\n");
        ...
        for (int i = 0; i < inventory.getWeapons().length; i++) {//does this for every weapon in inventory
            if (!inventory.getWeapons()[i].isEmpty()) {
                ...
                char bonusSign = ' ';
                boolean isStr;
                boolean isMonk = (Weapons.isSimpleMelee(inventory.getWeapons()[i]) && !(Weapons.isHeavy(inventory.getWeapons()[i]) ||
                        Weapons.isTwoHanded(inventory.getWeapons()[i]))) || inventory.getWeapons()[i].equalsIgnoreCase("Shortsword");
                ...
                if (stat.modifier[0] > stat.modifier[1])//determine attack bonus
                {
                    ...
                    if (Weapons.isDex(inventory.getWeapons()[i]) || isMonk) {
                        attackBonus += stat.modifier[1];
                        isStr = false;
                    } else {
                        attackBonus += stat.modifier[0];
...
//----------------------------------------------------------------------------------------------------------------------
    private void assignStats() {
        ...
        if (Character.rand.nextBoolean()) {
            ...
            assignedStats[2] = stats[3];//Assign Con as second highest stat.
        }
    ...
    public void createFirstLevel() throws IOException {//this is what runs when the class is selected.
        ...
        if (Character.rand.nextBoolean()) {//this determines equipment based on which ability score is dominant.
            inventory.addWeapon("Shortsword");
        } else {
        ...
        inventory.addWeapon("Dart");
        inventory.addMisc("10 Darts");

        features.addFeature("Unarmored Defense", "Beginning at 1st level, while you are wearing no " +
                "armor and not wielding a shield, your AC equals 10 + your Dexterity modifier + your Wisdom modifier.");
        //this adds a feature to the Features class
        armorClass = 10 + stat.getMods()[1] + stat.getMods()[4];

        features.addFeature("Martial Arts", "At 1st level, your practice of martial arts gives you " +
                "mastery of combat styles that use unarmed strikes and monk weapons, which are shortswords and any " +
                ...
                "one unarmed strike as a bonus action. For example, if you take the Attack action and attack with a " +
                "quarterstaff, you can also make an unarmed strike as a bonus action, assuming you haven’t already taken " +
    ...
    private String decideBackground()
    {
        ...
        int[] backgroundChance = {6, 4, 11, 2, 14, 4};
    ...
    public void decideSkills(int numberOfSkills) {//this determines what skills are assigned
        String[][] skillList = {//all skills are tied to a certain ability score. This multidimensional array pairs each skill with its respective score
                ...
                {},
                {"History", "Religion"},
                {"Insight"},//ie: all of these are Wisdom skills, so they are grouped together
                {}//whereas this is a Charisma skill, so it is separate.
        };
    ...
    private String[] assignWeaponProf() {//same for the weapons here
        String weaponList[] = {"Club", "Dagger", "Greatclub", "Handaxe", "Javelin", "Light Hammer", "Mace",
                "Quarterstaff", "Sickle", "Spear", "Light Crossbow", "Dart", "Shortbow", "Sling", "Shortsword"};
    ...
    private String[] assignToolProf(){
        String toolList[] = {"Bagpipes",  "Drum", "Dulcimer", "Flute", "Lute", "Lyre",  "Horn","Pan flute", "Shawm",
                "Viol", "Alchemist’s Supplies", "Brewer’s Supplies", "Calligrapher's Supplies",
        ...
        proficiency.addTool(toolList[Character.rand.nextInt(toolList.length)]);
        return toolList;
    }
